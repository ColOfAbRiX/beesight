...
object FlightStagesDetection {
  ...
  private val FreefallMinConsecutive         = 2     // points - require this many consecutive freefall points
  ...
  private val SpikeMaxAltitudeJump           = 100.0 // m per sample - max plausible altitude change
  private val SpikeMaxVerticalAccel          = 15.0  // m/s per sample - max plausible vertical acceleration change
  ...
  private def processPoint(state: StreamState, point: FlysightPoint, index: Long): StreamState =
    ...
    // Spike detection: check for physically implausible changes
    val verticalAccelChange = Math.abs(point.velD - state.verticalSpeed)
    val altitudeJump        = Math.abs(point.hMSL - state.height)
    val isSpike             = index > 0 && (verticalAccelChange > SpikeMaxVerticalAccel || altitudeJump > SpikeMaxAltitudeJump)
    ...
    // Track consecutive freefall points (reset on spike or non-freefall)
    val currentPoint = DataPoint(index, Some(point.hMSL))
    val (consecutiveCount, firstPoint) =
      if isSpike then
        (0, None)
      else if detectedPhase == DetectedPhase.Freefall then
        val newCount = state.consecutiveFreefallCount + 1
        val first    = state.firstFreefallPoint.orElse(Some(currentPoint))
        (newCount, first)
      else
        (0, None)
    ...
    StreamState(
      ...
      isSpike = isSpike,
      consecutiveFreefallCount = consecutiveCount,
      firstFreefallPoint = firstPoint,
    )
  ...
  private def tryDetectFreefall(takeoff: Option[DataPoint]): TryDetect[Option[DataPoint]] =
    Reader { (result, state, currentPoint) =>
      if result.freefall.isDefined then
        ...
        None
      else if state.consecutiveFreefallCount < FreefallMinConsecutive then
        None // Require sustained freefall to filter spikes
      else
  ...
  final private case class StreamState(
    ...
    consecutiveFreefallCount: Int = 0,
    firstFreefallPoint: Option[DataPoint] = None,
  )
